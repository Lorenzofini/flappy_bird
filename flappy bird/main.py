...
birdplayer_image = 'images/mario1.png'
	...
	while True:
		for event in pygame.event.get():
			if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
				...
				sys.exit()
			i = 10
			if event.type == KEYDOWN and (event.key == K_SPACE or event.key == K_UP or i == 10):
				if vertical > 0:
...
def isGameOver(horizontal, vertical, up_pipes, down_pipes):
	...
	for pipe in up_pipes:
		...
		abs(horizontal - pipe['x'] + 5) < game_images['pipeimage'][0].get_width()):
			return True
